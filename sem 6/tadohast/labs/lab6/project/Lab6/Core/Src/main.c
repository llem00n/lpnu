...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	...
	if (HAL_UART_Receive_IT(&huart2, &u8_DataBuffer[0], DataBufferSize) != HAL_OK)
	{
  ...
  while (1)
  {
		if (huart2.RxXferCount == 0)
		{
			...
			if (HAL_UART_Transmit_IT(&huart2, &u8_DataBuffer[0], DataBufferSize) != HAL_OK)
			{
				while (HAL_UART_GetState(&huart2) != HAL_UART_STATE_READY) {}
			}
		}
